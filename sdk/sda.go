...
type ResponseSdaGetMulticastDetailsFromSdaFabricMulticastVnInfo struct {
	...
	ExternalRpIPAddress string                                                               `json:"externalRpIpAddress,omitempty"` // External Rp Ip Address, required if multicastType is asm_with_external_rp
...
type RequestItemSdaUpdateDefaultAuthenticationProfileInSdaFabric struct {
	...
	Dot1XToMabFallbackTimeout string `json:"dot1xToMabFallbackTimeout,omitempty"` // Dot1x To MabFallback Timeout
...
type RequestSdaAddSiteInSdaFabric struct {
	FabricName        string `json:"fabricName,omitempty"`        // Should be an existing fabric name of your choice, otherwise "Default LAN Fabric" will be used as fabricName
	SiteNameHierarchy string `json:"siteNameHierarchy,omitempty"` // Pre-existing site name hierarchy available at global level
}
...
type RequestSdaAddPortAssignmentForUserDeviceInSdaFabric struct {
	...
	AuthenticateTemplateName  string `json:"authenticateTemplateName,omitempty"`  // Authenticate TemplateName associated to siteNameHierarchy ( In the case of scalableGroupName use only No Authentication is applied)
	ScalableGroupName         string `json:"scalableGroupName,omitempty"`         // Scalable Group name (for 2.2.2.6 allowed that scalableGroupName which associated with vn)
	InterfaceDescription      string `json:"interfaceDescription,omitempty"`      // Details or note of interface port assignment
}
...
type RequestSdaAddIPPoolInSdaVirtualNetwork struct {
	SiteNameHierarchy     string `json:"siteNameHierarchy,omitempty"`     // Path of sda Fabric Site
	VirtualNetworkName    string `json:"virtualNetworkName,omitempty"`    // Virtual Network Name, that is associated to Fabric Site
	IsLayer2Only          string `json:"isLayer2Only,omitempty"`          // Layer2 Only enablement flag
	...
	IsL2FloodingEnabled   *bool  `json:"isL2FloodingEnabled,omitempty"`   // Layer2 flooding enablement flag(applicable for L3 , L2 and always true for L2)
	IsThisCriticalPool    *bool  `json:"isThisCriticalPool,omitempty"`    // Critical pool enablement(applicable for L3)
	IsWirelessPool        *bool  `json:"isWirelessPool,omitempty"`        // Wireless Pool enablement flag(applicable for L3  and L2)
	IsIPDirectedBroadcast *bool  `json:"isIpDirectedBroadcast,omitempty"` // Ip Directed Broadcast enablement flag(applicable for L3)
	IsCommonPool          *bool  `json:"isCommonPool,omitempty"`          // Common Pool enablement flag(applicable for L3 and L2)
